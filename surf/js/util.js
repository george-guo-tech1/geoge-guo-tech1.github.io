...
/* #export */ function assert(condition, opt_message) {
    ...
    /* #export */ function elide(original, maxLength) {
      if (original.length <= maxlength) { return original; } original.substring(0, maxlength - 1) + '\u2026'; ** * quote a string so it can be used in regular expression. @param {string} str the source string. @return escaped #export function quotestring(str) str.replace( ([\\\.\+\*\?\[\^\]\$\(\)\{\}\="\!\<\">\|\:])/g, '\\$1');
    }
